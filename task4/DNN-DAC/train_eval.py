    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains, labels) in enumerate(train_iter):
            ...
            if total_batch % 100 == 0:
                ...
                # msg = 'Iter: {:>6},  Train Loss: {:>5.2},  Train Acc: {:>6.2%},  Val Loss: {:>5.2},  Val Acc: {:>6.2%},  Time: {} {}'
...
def test(config, model, test_iter):
    ...
    model.eval()
    start_time = time.time()
    p, r, f1, test_acc, test_loss, test_report, test_confusion = evaluate(config, model, test_iter, test=True)
...
def evaluate(config, model, data_iter, test=False):
    ...
    if test:
        ...
        return p, r, f1, acc, loss_total / len(data_iter), report, confusion
    return p, r, f1, acc, loss_total / len(data_iter)


def save_test(config, model, data_iter):
    """
    @TimeAgent
    保存为json文件
    """
    ...
    if not os.path.exists(path_):
        os.makedirs(path_)
    test_save_path = path_ + '/IMCS-DAC_test.json'
    shutil.copyfile(text_file_path, test_save_path)
    samples = load_json(test_save_path)
    i = 0
    for pid, sample in samples.items():
        for sent in sample:
            sent['dialogue_act'] = config.class_list[predict_all[i]]
            i = i + 1
